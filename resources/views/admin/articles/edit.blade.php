...
<div class="max-w-5xl mx-auto">
    ...
    <div class="bg-canvas rounded-[24px] border border-hairline shadow-card-lg overflow-hidden">
        <div class="px-6 py-5 border-b border-hairline bg-canvas-soft">
            <h2 class="text-lg font-bold text-ink tracking-tight">Edit Artikel</h2>
        </div>
        
        <form id="article-form" action="{{ route('admin.artikel.update', $article->id) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="judul" id="judul" value="{{ old('judul', $article->judul) }}" class="form-input w-full bg-white border-hairline rounded-xl py-2 px-3 focus:ring-primary focus:border-primary shadow-sm" required>
                </div>

                <div>
                    <label for="gambar_sampul" class="block text-[11px] font-bold text-mute uppercase tracking-widest mb-1.5">Gambar Sampul Baru (Opsional)</label>
                    @if($article->gambar_sampul)
                        <div class="mb-3">
                            <p class="text-xs text-mute mb-2">Gambar saat ini:</p>
                            <img src="{{ $article->gambar_sampul }}" alt="{{ $article->judul }}" class="h-32 rounded-lg border border-hairline object-cover">
                        </div>
                    @endif
                    ...
                    <p class="text-xs text-mute mt-1">Format: JPG, PNG, GIF. Maksimal 5MB. Biarkan kosong jika tidak ingin mengubah gambar.</p>
                </div>
                
                <div>
                    <label class="block text-[11px] font-bold text-mute uppercase tracking-widest mb-1.5">Isi Artikel</label>
                    <input type="hidden" name="isi" id="isi_hidden" value="{{ old('isi', $article->isi) }}">
                    <div id="editor-container">{!! old('isi', $article->isi) !!}</div>
                </div>
                ...
                <div>
                    ...
                    <select name="status" id="status" class="form-select w-full bg-white border-hairline rounded-xl py-2 px-3 focus:ring-primary focus:border-primary shadow-sm" required>
                        <option value="draft" {{ (old('status') == 'draft' || (!$article->sudah_diterbitkan && old('status') === null)) ? 'selected' : '' }}>Simpan sebagai Draft</option>
                        <option value="published" {{ (old('status') == 'published' || ($article->sudah_diterbitkan && old('status') === null)) ? 'selected' : '' }}>Terbitkan Langsung</option>
                    </select>
                </div>
            </div>
