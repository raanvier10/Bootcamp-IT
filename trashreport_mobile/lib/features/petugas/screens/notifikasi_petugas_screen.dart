...
class NotifikasiPetugasScreen extends StatefulWidget {
  @override
  _NotifikasiPetugasScreenState createState() => _NotifikasiPetugasScreenState();
}

class _NotifikasiPetugasScreenState extends State<NotifikasiPetugasScreen> {
  ...
  Future<void> _fetchNotifikasi() async {
    ...
    try {
      final response = await _apiClient.dio.get('/petugas/notifikasi');
  ...
  Future<void> _markAsRead(int id, int index) async {
    try {
      await _apiClient.dio.post('/petugas/notifikasi/$id/baca');
      setState(() {
  ...
  Color _getIconColor(String tipe) {
    ...
    if (tipe.toLowerCase() == 'error' || tipe.toLowerCase() == 'ditolak') return const Color(0xFFDC2626);
  ...
  IconData _getIcon(String tipe) {
    ...
    if (tipe.toLowerCase() == 'error' || tipe.toLowerCase() == 'ditolak') return Icons.cancel_rounded;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Notifikasi Petugas', style: GoogleFonts.outfit(color: textPrimary, fontWeight: FontWeight.bold, fontSize: 18)),
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: const Icon(Icons.assignment_turned_in_outlined, size: 64, color: Color(0xFF9CA3AF)),
          ),
          const SizedBox(height: 24),
          Text('Belum Ada Tugas Baru', style: GoogleFonts.outfit(fontSize: 20, fontWeight: FontWeight.bold, color: const Color(0xFF111827))),
          const SizedBox(height: 8),
          Text('Saat ini tidak ada notifikasi penugasan\natau pembaruan sistem.', textAlign: TextAlign.center, style: GoogleFonts.outfit(fontSize: 14, color: const Color(0xFF6B7280), height: 1.5)),
        ],
      ),
    );
  }
